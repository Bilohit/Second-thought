...
export default function CompactHistory({ onOpenFile }: Props) {
  ...
  return (
    <div style={{ height: "100%", minWidth: 0, display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" }}>
      ...
      <div style={{ flex: "1 1 auto", minHeight: 160, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "10px 12px", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ fontSize: 10, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8, display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          Recent activity
          {rows.length > 0 && (
          ...
          {rows.map((row) => (
            <button
              ...
              style={{
                ...
                padding: "7px 4px", cursor: "pointer", border: "none", borderBottom: "1px solid var(--border-2)",
                background: "transparent", textAlign: "left", fontFamily: "inherit",
              }}
            >
              <span style={{ fontSize: 12, color: "var(--text-1)", flex: 1, minWidth: 0, lineHeight: 1.45, wordBreak: "break-word" }}>
