...
describe("formatChatFailure", () => {
  it("prefixes an Error's message with the ⚠ convention", () => {
...
describe("getRetryTarget", () => {
  ...
  it("recovers the stored userQuery and drops the failed user+assistant pair", () => {
    const messages: ChatMessage[] = [
      ...
      { role: "assistant", content: "⚠ Chat failed: connection lost", userQuery: "what is the vault path" },
    ];
  ...
  it("preserves earlier turns ahead of the failed pair", () => {
    const messages: ChatMessage[] = [
      ...
      { role: "assistant", content: "⚠ Chat failed: connection lost", userQuery: "second question" },
    ];
