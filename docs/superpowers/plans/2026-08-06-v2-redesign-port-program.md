...
**The geometry check that makes or breaks this phase:** re-measure `.capsule-label` and compare against Phase 0's baseline. `CAPSULE_CLOSED_W` stays **154** — frozen. If "Second Thought" no longer fits the 98px text budget, solve it as a *fit* problem (tracking, size) inside the frozen width. **Updating `CAPSULE_TEXT_W` moves the morph and is forbidden.**
...
1. **The typeface is a geometry change.** Verified at `CapsuleMenu.tsx:39`. Phase 2's measurement gate is the control.
